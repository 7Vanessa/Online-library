...
router.get('/panier', (req, res) => {
    ...
    db.query(`SELECT livre.id_livre, livre.titre, livre.image, livre.auteur, items.quantity FROM livre INNER JOIN (SELECT * FROM panier_item WHERE panier_item.id_panier_i=(SELECT panier.id_panier FROM panier WHERE panier.id_user_panier=` + parseInt(req.session.userId) + `)) as items ON livre.id_livre=items.id_livre_item`, function(err, result) {
      if(err) {
        throw err;
      };
...
router.post('/panier', (req, res) => {
  ...
  const idArticle = parseInt(req.body.id_livre)
  const quantityArticle = parseInt(req.body.quantity)

  db.query(`INSERT INTO panier_item VALUES (0, ` + idArticle + `, ` + quantityArticle + `, ` + req.session.userId + `)`, function(err, result) {
    if(err) {
      throw err;
    };
...
router.post('/panier/pay', (req, res) => {
  ...
  const liste_livre = req.body.livres

  const taille = liste_livre.length

  for(let index=0; index<taille; index++) {
    db.query(`UPDATE livre SET nb_exemplaires=(nb_exemplaires-` + liste_livre[index].quantity + `) WHERE id_livre=` + liste_livre[index].id_livre, function (err, result) {
      if (err) {
        throw err;
      }
    })
  ...
  db.query('DELETE FROM panier_item WHERE id_panier_i=' + req.session.userId, function(err, result) {
    if(err) {
      throw err;
    };
 ...
 */
router.post('/panier/:articleId', (req, res) => {

  const id = req.body.id_livre

  console.log(id)

  db.query('DELETE FROM panier_item WHERE id_livre_item=' + id, function(err, result) {
    if(err) {
      throw err;
    };
  })
...
/**
 * Cette route gère la suppression d'un article en base de données
 */
router.post('/articles/:articleId', (req, res) => {
  const id_livre = parseInt(req.body.id_livre)

  console.log(id_livre)
  console.log(typeof id_livre)

  if(isNaN(id_livre)) {
    return
  }

  db.query('DELETE FROM livre WHERE id_livre=' + id_livre, function(err, result) {
    if(err) {
      throw err;
    };
    return
...
router.post('/login', (req, res) => {
  ...
  db.query(`SELECT * FROM user WHERE ('` + user.email + `' LIKE email) AND ('` + user.mdp + `' LIKE password)`, function(err, result) {
    if(err) {
      throw err;
    };
    ...
    db.query(`UPDATE user SET actif=true WHERE id_user=` + req.session.userId, function(err, result) {
      if(err) {
        throw err;
      };
    ...
    db.query(`SELECT id_panier FROM panier WHERE id_panier=` + req.session.userId, function(err, result) {
      if(err) throw err;
      req.session.panierId=result[0].id_panier
      console.log('id panier: ' + req.session.panierId)
    })
...
/*
 * Cette route gère la récupération d'un utilisateur
 */
router.get('/user',(req,res) => {
  ...
  db.query(`SELECT * FROM user WHERE id_user=` + req.session.userId, function(err, result) {
    if(err) {
      throw err;
    };
...
/*
 * Cette route gère la déconnexion d'un utilisateur
 */
router.get('/logout',(req,res) => {
  db.query(`UPDATE user SET actif=false WHERE id_user=` + req.session.userId, function(err, result) {
    if(err) {
      throw err;
    };
  })
...
router.post('/signin', (req, res) => {
  ...
  db.query(`SELECT id_user FROM user WHERE nom LIKE '` + user_nom + `' AND prenom LIKE '` + user_prenom + `'`, function(err, result) {
    if(err) throw err;
    const id = result[0].id_user

    //creation d'un panier
    db.query(`INSERT INTO panier VALUES (0, ` + id + `)`, function(err, result) {
      if(err) throw err;
      console.log(result);

    })
    db.query(`SELECT id_panier FROM panier WHERE id_panier=` + id, function(err, result) {
      if(err) throw err;
      req.session.panierId=result[0].id_panier
    })
  })
